...
predict_wrapper <- function(rdtsdF,style,newdata,newdatalocs,spatialblend){
     ...
     newdatapredsfull=ddply(rdtsdF,.(yrdoy),predict_surfaces,snotellocs.usgs,newdata,newdatalocs,spatialblend,bpth,.inform=T,.parallel=F)#,.paropts=list(.export=ls(), .packages=.packages(all=T)))     
...
if(output!='points'){
     ...
     fn2w=paste0('diagnostics/rswe_',recon.version,'/fullpreds/snotellocs/snotel_predictions_',unique(rdtsdF$yr),'.txt')
