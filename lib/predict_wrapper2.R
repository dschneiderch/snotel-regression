...
predict_wrapper <- function(rdtsdF,style,newdata,newdatalocs,newdatalocs.agg.usgs,spatialblend,scalesnotel,fscaMatch,cost,fordensource,dateflag,predictor){
     ...
     phvrcn.fn=file.path(bpth,cost,'netcdf',style,paste0('preds-phv',predictor,'_',yr,'_',spatialblend,'-',fscaMatch,'.nc'))
     ...
     d_ply(rdtsdF,.(yrdoy),function(doyinfo){
          dF=predict_surfaces(doyinfo,snotellocs,snotellocs.usgs,newdata,newdatalocs,newdatalocs.agg.usgs,spatialblend,bpth,scalesnotel,fordensource,dateflag,predictor)#,.inform=F,.parallel=F)#,.paropts=list(.export=ls(), .packages=.packages(all=T)))
