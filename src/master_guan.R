...
spatialblend='krige300km'
covrange=300000
lapply(as.list(ryrs),function(yr){
	recondates=data.frame(date=seq(as.POSIXct(paste0(yr,'-03-01'),as.POSIXct(paste0(yr,'-08-31')))))
	recondates$yr=strftime(recondates$date,'%Y')
	recondates$yrdoy=strftime(recondates$yrdoy,'%j')
	d_ply(recondates,.(yr),blend_recon,newdatalocs.usgs,newdatalocs.agg.usgs,spatialblend,recon.version,covrange,.inform=F,.parallel=F,.paropts=list(.export=ls(), .packages=.packages(all=T)))
}
