...
### Base model
            ...
            mdl=glm(snotel ~ Lat+Elev+Eastness+Northness+Slope+RegionalSlope+RegionalEastness+RegionalNorthness+FtprtW+Wbdiff+NWbdiff+Wd2ocean+1,family=gaussian(link='log'),data=swe2model)
#                  
### Reduce with stepaic
            ## PHV only
...
### Reduce with stepaic
            ...
            newformula=paste(mdl$formula[2],mdl$formula[1],mdl.stepaic.phv$formula[3],'+ recon',sep=' ')
                                        #print(newformula)
            mdl.wrecon=glm(newformula,data=swe2model,family=gaussian(link='log'))
                                        #                
